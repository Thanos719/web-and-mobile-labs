...
if (!isset($_SESSION['user_id'])) {
    header('Location: login.php'); // Ανακατεύθυνση στη σελίδα login εάν ο χρήστης δεν έχει συνδεθεί
    exit();
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Εισαγωγή νέας καταχώρησης στον πίνακα listings
    ...
    if (mysqli_query($conn, $query)) {
        header('Location: feed.php'); // Μεταφορά στη σελίδα feed μετά την επιτυχή εισαγωγή
    } else {
        $error = "Error: " . mysqli_error($conn); // Αναφορά σφάλματος σε περίπτωση αποτυχίας
    }
}
?>
...
<head>
    ...
    <script>
        function validateForm() {
            ...
            var regex = /\d/; // Έλεγχος περιεχομένου αριθμών
